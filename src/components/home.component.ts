...
@Component({
  selector: 'app-home',
  imports: [RouterLink, NgOptimizedImage, CommonModule, FormsModule, KanaToRomajiPipe],
  template: `
...
export class HomeComponent {
  ...
  onSearch() {
    ...
    // 1. Search Kanji (Char, Meaning, Readings)
    ...
    // 2. Search Vocab (Word, Kana, Meaning)
  ...
  closeSearch() {
    // Delay slightly to allow click event on result to register first
    setTimeout(() => {
      // Optional: Logic to hide dropdown on outside click if needed
      // For now, handled by layout structure
    }, 100);
  }
